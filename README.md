...
I learned much about the concurrency and optimization methods that are frequently used in data-intensive apps. Seeing the difference in time that multithreading and caching made on my own project was quite eye-opening. Additionally, I learned much about implementing security measures for such an app, particularly rate-limiting and text sanitization/preprocessing.
...
To use the AI analyzer, simply input a single valid replay link or replay ID, or multiple valid replay links/IDs (if inputting multiple, separate each replay by a comma or a new line/enter). Users are rate limited to 4 AI summary uses per hour (for now).
