...
if __name__ == "__main__":
    from replay_parser import fetch_and_parse_replay_stats
    TEST_REPLAY_ID = "gen9ou-2657015138"
    TARGET_USER = "harrolux"

    print(f"1. Fetching and parsing {TEST_REPLAY_ID} for user '{TARGET_USER}'...")
    parsed_payload = fetch_and_parse_replay_stats(TEST_REPLAY_ID, TARGET_USER)

    if not parsed_payload:
        print("Failed to fetch or parse replay. Double check the replay ID and username.")
    else:
        print("2. Raw Parsed Payload:")
        print(json.dumps(parsed_payload, indent=2))

        print("\n3. Sending parsed payload to Gemini 3.6 Flash...")
        analysis_result = analyze_single_game(parsed_payload)

        print("\n4. Gemini Analysis Output:")
...
if __name__ == "__main__":
    ...
    print(f"1. Fetching {len(TEST_REPLAY_IDS)} replays concurrently for '{TARGET_USER}'...")
    parsed_results = fetch_stats_concurrently(TEST_REPLAY_IDS, TARGET_USER)
    
    # Filter out any failed parses
    game_payloads = [payload for payload in parsed_results.values() if payload]

    if not game_payloads:
        print("No valid replays were parsed.")
    else:
        print(f"2. Successfully parsed {len(game_payloads)} games. Sending aggregate to Gemini...")
        multi_analysis = analyze_multi_games(TARGET_USER, game_payloads)

        print("\n3. Multi-Game Coaching Report:")
        print(multi_analysis.model_dump_json(indent=2))